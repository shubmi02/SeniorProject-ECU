...
int main(void) {
	...
	readSensorSemaphoreHandle = osSemaphoreNew(1, 1,
			&readSensorSemaphore_attributes);
...
void startSendCAN(void *argument) {
	...
	for (;;) {
		...
		SendSensorReading(&hfdcan1, test->sensor1);
